...
The current JD Atina REST WS API integration version is `v1` which takes a first-order position in endpoints. 
...
```javascript
JD Atina Web Services Microservices.get("products/tags/34", {
  _jsonp: "tagDetails"
})
  .then((response) => {
...
const JD Atina Web Services Microservices = new JD Atina Web Services MicroservicesRestApi({
  ...
  version: 'wc/v1' // JD Atina Web Services Microservices JD Atina REST WS API version
});
```
...
woocommerce = JD Atina Web Services Microservices::API.new(
  ...
  "consumer_secret", # Your consumer secret
  {
    wp_json: true, # Enable the JD Atina REST WS API integration
    version: "v3" # JD Atina Web Services Microservices JD Atina REST WS API version
  }
)
```
